...
import type { WarehouseCell, StockRow } from '../../../data/demoWmsData'
...
export default function StockModule({ grid, stockRows, track }: { grid: WarehouseCell[]; stockRows: StockRow[]; track: (id: string) => void }) {
          ...
          <div style={{ background: 'var(--dark-2)', border: '1px solid rgba(27,53,84,0.08)', padding: '1.75rem' }}>
            ...
            {selectedCell ? (
              <motion.div key={selectedCell.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, ease }}>
                <div style={{ fontFamily: 'Manrope, sans-serif', fontSize: '1.5rem', fontWeight: 800, color: 'var(--navy)', marginBottom: '1rem' }}>{selectedCell.id}</div>
      ...
      <div>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 640 }}>
            ...
            <tbody>
              {filtered.map((row) => (
                <StockTr key={`${row.reference}-${row.emplacement}`} row={row} />
              ))}
              {filtered.length === 0 && (
                <tr>
